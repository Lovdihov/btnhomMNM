    ...
    <div class="mb-10">
        ...
        <div class="bg-[#1e2038] rounded-2xl overflow-hidden shadow-xl">
            @foreach($topSongs as $index => $song)
                ...
                {{-- 1. ĐƯA ONCLICK LÊN DIV BỌC NGOÀI CÙNG CỦA BÀI HÁT --}}
                <div onclick="playSong('{{ $song->audio_url }}', '{{ addslashes($song->title) }}', '{{ $coverUrl }}', '{{ $artistNames }}', {{ $song->id }})" 
                     class="flex items-center justify-between p-4 bg-[#1e2038] hover:bg-white/10 rounded-xl mb-3 border border-gray-800/50 transition-all group shadow-sm hover:shadow-md cursor-pointer">
                    ...
                    <div class="flex items-center gap-2 sm:gap-4 pl-4">
                        ...
                        {{-- 2. THÊM event.stopPropagation() VÀO NÚT THẢ TIM --}}
                        <button @class([
                        ...
                        {{-- 3. XÓA ONCLICK Ở NÚT PLAY VÌ ĐÃ CÓ Ở THẺ DIV CHA --}}
                        <button class="bg-purple-600 text-white w-10 h-10 flex items-center justify-center rounded-full hover:bg-purple-500 hover:scale-110 transition-all shadow-lg shadow-purple-500/30">
